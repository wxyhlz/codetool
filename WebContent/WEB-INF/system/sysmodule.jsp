...
<%--	<!-- OTHER FRAME... -->	--%>
	...
	<script type="text/javascript">
		...
		function getGridData(vQueryType,vParams) {
			...
			var url = path+'/sysmodule/findSysmoduleByPage.action';
			$.ajax({
        		type:"POST",
       			url:url,
        		data:params,
		...
		function initGrid(){
			$('#tt').treegrid({
				...
				columns:[[  
					...
					{title:'URL',field:'actionurl',width:200,rowspan:2,align:'center'},						
					...
					{title:'打开方式',field:'opentypeStr',width:120,rowspan:2,align:'center',
						formatter:function(value,rowData,rowIndex){
							var s="";
							if(rowData.opentype == "1"){
								s = "平铺";
							}else if(rowData.opentype == "0"){
								s = "弹出";
							}
							return s;
						}
		...
		function deleteData(rowid){
			$.messager.confirm('友情提醒','请确认是否删除所选记录?',function(r){
				if(r){
					...
					var url = path+'/sysmodule/delete.action';
					$.ajax({
		          		type:"POST",
		          		data:params,
		         		url:url,
		...
		function btnInsert(){ 			
			...
			params.actionurl = $("#actionUrl").val();

			//如果不是根节点			
			if(!$("#rootNode").attr("checked")){
				//为空判断
				var elename = "name,actionUrl";
				var elehint = "名称,URL";
				...
				if((row==undefined)||(row==null)){
					 $.messager.alert('友情提醒',"请选择要插入的行。",'info'); 
					return;
				}
				//赋值
				params.pid = row.id;
			...
			var url = path+'/sysmodule/insert.action'; 
			$.ajax({
        		type:"POST",
       			url:url,
        		data:params,
...
<%--		    	$('#actionUrl').val(row.url);--%>
<%--		    	$("#rootNode").attr("checked",true);--%>
<%--		    	$("#rootNode").attr("disabled",true);--%>
...
<%--		    }--%>
		...
		function btnUpdate(){
			...
			var url = path+'/sysmodule/update.action'; 
			$.ajax({
        		type:"POST",
       			url:url,
        		data:params,
				...
				<table style="border-collapse:collapse;margin-top:5px;" bordercolor=#99ccff cellspacing=0 rules=rows width=300 align=center border=1>
					<tbody>
						<tr style="height:26px;">
							<td width=100 style="text-align:right;">模块名称：</td>
		...
		<form id="ff" method="post">
			<table>  
				...
				<tr height=28>
					<td align=right></td>
					<td> 
						<input type="checkbox" id="rootNode" name="rootNode" checked="checked"/>根功能点
						<input type="checkbox" id="openType" name="openType" checked="checked"/>打开方式：弹出
					</td>
					<td></td>
				</tr> 
				<tr height=28>
				...
				<tr height=28>
					...
					<td>
						<input class="input22" type="text" id="actionUrl" name="actionUrl" validType="length[0,199]"  style="width:260px"></input>
					</td>
					<td><span style="font-size:16px;color:red;">*</span></td>
				</tr> 
			</table> 
