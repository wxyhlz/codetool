 ...
 * eg.
 * $.ajaxFileUpload({
 * 		url:'upBussinessAttach.action',//改用于文件上传的服务器端请求地址 
 *      secureuri:false,//一般设置为false
 * 		fileElementId:'file',//文件上传空间的id属性  <input type="file" id="file" name="file" />
 * 		dataType: 'json',//返回值类型 一般设置为json
 * 		data: {"fileName":fileName,"businessID":businessID,"actionStatus":actionStatus}, 
 * 		success: function (data, status){  //服务器成功响应处理函数						
 * 			var analyMsg = data;
 * 			if(!analyMsg.successed){ 
 * 				$.messager.alert('友情提醒',analyMsg.errors[0],'info',function(){});
 * 			}else{ 
 * 				$.messager.alert('友情提醒','上传原始凭证成功！','info',function(){ 
 * 					$("#tt").datagrid("loadData", analyMsg.returnObject);
 * 					businessID = analyMsg.returnObject.rows[0].aljjyw_id; 
 * 				}); 										   		
 * 			}						
 * 		},
